...
class Config:
    def __init__(self, args):
        ...
        self.save_model = f"{self.arch}_{self.stage}_{self.sparsity_type}_{self.run_id}.pth.tar"

        if self.stage == 'retrain':
            self.load_model = f"{self.arch}_{'admm'}_{self.sparsity_type}_{self.run_id}.pth.tar"
        elif self.stage == "admm":
            if self.arch == "resnet18":
                self.load_model = "resnet18_pretrain_cifar10.pth.tar"
            elif self.arch == "wrn_28_4":
                self.load_model = "wrn284_pretrain_cifar10.pth.tar"
            elif self.arch == "vgg16_bn":
                self.load_model = "vgg16_bn_pretrain_cifar10.pth.tar"
        else:
        ...
        ds_name = 'CIFAR10'
        radmm_dir = os.getcwd()
        strategy_f = '/'.join(radmm_dir.split('/')[:-3] + ['strategies/{}.json'.format(ds_name)])
        strategies = json.load(open(strategy_f))

        arch = self.arch

        heracles_stg = strategies[arch][self.sparsity_type][self.run_id]

        self.prune_stg = [[], [], []]  # [[conv_prates], [fc_prates], [shortcut-prates]]
        if self.arch == 'vgg16_bn':
            assert len(heracles_stg) == 16, '! ! {}-{}-{} is invalid'.format(self.arch, self.sparsity_type, self.run_id)
            self.prune_stg[0] = heracles_stg[:13]
            self.prune_stg[1] = heracles_stg[13:]
            self.prune_stg[2] = []
        elif self.arch == 'resnet18':
            if self.sparsity_type == 'channel':
                assert len(heracles_stg) == 18, '! ! {}-{}-{} is invalid'.format(self.arch, self.sparsity_type, self.run_id)
                self.prune_stg[0] = heracles_stg[:17]
                self.prune_stg[1] = heracles_stg[17:]
                self.prune_stg[2] = list(np.take(heracles_stg, (5, 9, 13)))
            if self.sparsity_type == 'weight':
                assert len(heracles_stg) == 21, '! ! {}-{}-{} is invalid'.format(self.arch, self.sparsity_type, self.run_id)
                self.prune_stg[0] = heracles_stg[:5] + heracles_stg[6:10] + heracles_stg[11:15] + heracles_stg[16:20]
                self.prune_stg[1] = heracles_stg[20:]
                self.prune_stg[2] = list(np.take(heracles_stg, (5, 10, 15)))
        elif self.arch == 'wrn_28_4':
            if self.sparsity_type == 'channel':
                assert len(heracles_stg) == 26, '! ! {}-{}-{} is invalid'.format(self.arch, self.sparsity_type, self.run_id)
                self.prune_stg[0] = heracles_stg[:25]
                self.prune_stg[1] = heracles_stg[25:]
                self.prune_stg[2] = list(np.take(heracles_stg, (1, 9, 17)))
            if self.sparsity_type == 'weight':
                assert len(heracles_stg) == 29, '! ! {}-{}-{} is invalid'.format(self.arch, self.sparsity_type, self.run_id)
                self.prune_stg[0] = heracles_stg[:1] + heracles_stg[2:10] + heracles_stg[11:19] + heracles_stg[20:28]
                self.prune_stg[1] = heracles_stg[28:]
                self.prune_stg[2] = list(np.take(heracles_stg, (1, 10, 19)))
        else:
            raise NameError('Strategy check only supports vgg16_bn, resnet18, wrn_28_4 but no "{}"'.format(self.arch))
        ...
        try:
            with open(config_dir, "r") as stream:
                ...
                if stage != 'pretrain':
                    ...
                    if not self.rate_from_config:
                        if self.uniform:
                            ...
                            self.insert_nonuniform_strategy()
                ...
                self.warmup_epochs = raw_dict['retrain']['warmup_epochs']
                self.warmup_lr = raw_dict['retrain']['warmup_lr']
    ...
    def insert_nonuniform_strategy(self):
        ...
        print(f"Replaced uniform strategy with Heracles strategy")
